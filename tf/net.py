...
class Net:
    ...
    def set_networkformat(self, net):
        ...
        if (
            net == pb.NetworkFormat.NETWORK_ATTENTIONBODY_WITH_HEADFORMAT
            and self.pb.min_version.minor < LC0_MINOR_WITH_ATTN_BODY
        ):
            self.pb.min_version.minor = LC0_MINOR_WITH_ATTN_BODY
        if (
            net == pb.NetworkFormat.NETWORK_ATTENTIONBODY_WITH_MULTIHEADFORMAT
    ...
    def set_input(self, input_format):
        ...
        if (
            input_format == pb.NetworkFormat.INPUT_112_WITH_CANONICALIZATION_V2
            or input_format
            == pb.NetworkFormat.INPUT_112_WITH_CANONICALIZATION_V2_ARMAGEDDON
        ):
            self.pb.min_version.minor = LC0_MINOR_WITH_INPUT_TYPE_5
        elif (
            input_format >= pb.NetworkFormat.INPUT_112_WITH_CANONICALIZATION_HECTOPLIES
        ):
    ...
    def tf_name_to_pb_name(self, name):
        ...
        if base_layer == "policy":
            # Check if policy weights are at root level (pol_headcount=0, T1-style)
            # or under policy_heads (newer multi-head style)
            use_policy_heads = self.pb.weights.pol_headcount > 0 or (
                hasattr(self.pb.weights, "policy_heads")
                and hasattr(self.pb.weights.policy_heads, "vanilla")
                and len(
                    self.denorm_layer_v2(self.pb.weights.policy_heads.vanilla.ip2_pol_w)
                )
                > 0
            )
            ...
            if layers[1] == "embedding":
                if layers[2].split(":")[0] == "kernel":
                    pb_name = pb_prefix + "ip_pol_w" if pb_prefix else "ip_pol_w"
                else:
                    pb_name = pb_prefix + "ip_pol_b" if pb_prefix else "ip_pol_b"

            elif layers[1] in ["vanilla", "soft", "optimistic_st", "opponent", "next"]:
                if use_policy_heads:
                    pb_prefix = pb_prefix + layers[1] + "."
                # For T1-style (no policy_heads), vanilla maps directly to root
                if layers[2] == "attention":
                    pb_name = attn_pol_to_bp(layers[3], weights_name)
                pb_name = pb_prefix + pb_name if pb_prefix else pb_name

        elif base_layer == "value":
            # Check if value weights are at root level (T1-style) or under value_heads
            use_value_heads = (
                hasattr(self.pb.weights, "value_heads")
                and hasattr(self.pb.weights.value_heads, "winner")
                and len(
                    self.denorm_layer_v2(self.pb.weights.value_heads.winner.ip_val_w)
                )
                > 0
            )
            ...
            if layers[1] in ["st", "q", "winner"]:
                ...
                # For T1-style (no value_heads), winner maps directly to root
    ...
    def get_weights_v2(self, names):
        ...
        for tf_name in names:
            ...
            if pb_name is None:
                raise ValueError(
                    "Don't know where to store weight in protobuf: {}".format(name)
                )
    ...
    def fill_net_v2(self, all_weights):
        ...
        for name, weights in all_weights:
            ...
            if (
                self.pb.format.network_format.input
...
def main(argv):
    ...
    if argv.input.endswith(".txt"):
        ...
        print(
            "Unable to detect the network format. "
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        "-i", "--input", type=str, help="input network weight text file"
    )
    argparser.add_argument(
        "-o", "--output", type=str, help="output filepath without extension"
    )
